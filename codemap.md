...
- `plugin/tui.tsx` — TUI plugin entry (exports `./tui`): `/stats` command, live status-line slot, loaded by opencode's TUI host from `tui.json` as a separate module.
...
- `CONTEXT.md` — domain vocabulary (family, LLM step, TTFT/TPS, quantization terms).
