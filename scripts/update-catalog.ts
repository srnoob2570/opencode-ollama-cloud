...
export function parseLibraryPage(html: string) {
  ...
  if (infoLine) {
    ...
    result.context = Math.round(
      infoLine[3] === "K" ? n * 1024 : n * 1024 * 1024,
    );
  ...
  const cardRe =
    /text-neutral-800">([\w.\-]+:[\w.\-]+)<\/p>[\s\S]{0,600}?text-neutral-500">([^<]+)<\/p>/g;
...
function mergeSeed(
  ...
  return catalogModels.map((m) => {
    const s =
      seedPick(prioritized, m.id, m.family) ?? seedPick(all, m.id, m.family);
    return {
      ...m,
      ...
      reasoningOptions:
        s?.reasoning_options
          ?.map((r: any) => r?.values)
          .flat()
          .filter(Boolean) ?? m.reasoningOptions,
...
async function loadCatalog(): Promise<Catalog> {
  ...
  try {
    ...
    console.warn(
      "warning: catalog/catalog.json is unparsable; regenerating from scratch",
    );
  }
...
async function main() {
  ...
  const modes = [
    ...new Set(
      args.filter((a): a is Mode => (MODES as readonly string[]).includes(a)),
    ),
  ];
  if (unknown.length > 0 || modes.length > 1) {
    ...
    if (modes.length > 1)
      console.error(`conflicting modes: ${modes.join(", ")}`);
    console.error(
      "usage: bun scripts/update-catalog.ts [check|update] [--force]",
    );
    process.exit(1);
  }
  ...
  if (mode === "check" && force)
    console.warn("warning: --force has no effect in check mode");
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(SCRAPE_CONCURRENCY, families.length) },
      async () => {
  ...
  for (const [family, variants] of byFamily) {
    ...
    for (const variant of variants) {
      ...
      models.push({
        ...
        reasoningOptions: [],
        releaseDate: new Date(variant.created * 1000)
          .toISOString()
          .slice(0, 10),
      });
    }
...
async function enrichQuantization(
  models: CatalogModel[],
  previous: Catalog,
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(QUANT_CONCURRENCY, models.length) },
      async () => {
          ...
          try {
            ...
            if (typeof digest === "string" && digest.length > 0) {
              const blob = await fetchJson<unknown>(
                REGISTRY_BLOB_URL(model.family, digest),
              );
  ...
  const enriched = models.map((m, index) => {
    ...
    if (q.warning)
      warnings.push(
        `${q.warning}: registry file_type came back empty — keeping the previous catalog value (check for an Ollama-side change)`,
      );
    return {
      ...m,
      ...
      ...(q.conflict
        ? { conflicts: { ...m.conflicts, quantization: q.conflict } }
        : {}),
    };
  });
