...
export function quantizationFromShow(show: unknown): string | null {
  ...
  const value: unknown = (details as Record<string, unknown>).quantization_level
  if (typeof value !== "string") return null
  return value.trim() === "" ? null : value
}
...
export function resolveQuantization(input: QuantizationInput): QuantizationResult {
  const registryValue = input.registry && input.registry.trim() !== "" ? input.registry : null
  ...
  if (registryValue) {
    if (showValue && showValue !== registryValue) {
      return {
        ...
        conflict: { registry: registryValue, "api/show": showValue, resolver: "registry-wins" },
      }
    }
