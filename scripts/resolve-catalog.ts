...
export interface OverrideEntry extends Pick<
  ModelPricing,
  "provider" | "source" | "asOf" | "note"
> {
  input: number;
  output: number;
}
...
const keysFor = (id: string): string[] => {
  return [
    ...new Set(
      [UPSTREAM_ALIASES[id], id, familyOf(id)].filter(
...
function marketCost(
  ...
  const counts = new Map<
    string,
    { cost: { input: number; output: number }; n: number }
  >();
  for (const provider of Object.values<any>(seed)) {
    ...
    if (prev) prev.n += 1;
      ...
      counts.set(k, { cost: { input: pick.input, output: pick.output }, n: 1 });
  }
    ...
    if (
      ...
      (entry.n === best.n &&
        (entry.cost.input < best.cost.input ||
          (entry.cost.input === best.cost.input &&
            entry.cost.output < best.cost.output)))
    )
      best = entry;
  }
  return best?.cost;
...
export function resolveCatalog(
  ...
  for (const m of models) {
    ...
    const ov = isOverride(rawOverride)
      ? (rawOverride as OverrideEntry)
      : undefined;
    ...
    if (ov) {
      ...
      if (
        seedPick &&
        (seedPick.input !== ov.input || seedPick.output !== ov.output)
      ) {
        conflicts = {
          pricing: {
      ...
      if (mode) {
        ...
        sources = { pricing: "models.dev" };
        warnings.push(
          `${m.id}: using marketplace mode price — consider adding an override`,
        );
      } else {
        warnings.push(
          `${m.id}: no reference price available (no override, no first-party entry)`,
        );
      }
    }
