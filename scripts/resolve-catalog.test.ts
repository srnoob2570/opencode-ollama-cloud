...
const SEED = {
  zai: {
    models: { "glm-5.3-flash": { cost: { input: 0.075, output: 0.25 } } },
  },
  moonshotai: { models: { "kimi-k3": { cost: { input: 3, output: 15 } } } },
  deepseek: {
    models: { "deepseek-v4-pro": { cost: { input: 0.435, output: 0.87 } } },
  },
  alibaba: {
    models: { "qwen3.5-397b-a17b": { cost: { input: 0.6, output: 3.6 } } },
  },
  nvidia: {
    models: {
  ...
  provider_unrelated: {
    models: { "glm-5.3-flash": { cost: { input: 9, output: 9 } } },
  },
};
...
describe("resolveCatalog overrides", () => {
  ...
  test("invalid overrides (zero/negative or absent numbers) are ignored, not trusted", () => {
    ...
    const models = resolve(
      [model("kimi-k3"), model("glm-5.3-flash")],
      overrides,
    ).models;
...
describe("resolveCatalog marketplace mode", () => {
  test("no first-party entry falls back to the most common real price, marked derived", () => {
    const seed = {
      ...SEED,
      host_a: {
        models: { "gpt-oss:120b": { cost: { input: 0.3, output: 1.2 } } },
      },
      host_b: {
        models: { "gpt-oss:120b": { cost: { input: 0.15, output: 0.6 } } },
      },
      host_c: {
        models: { "gpt-oss:120b": { cost: { input: 0.15, output: 0.6 } } },
      },
    };
    const { models, warnings } = resolveCatalog([model("gpt-oss:120b")], {
      seed,
      overrides: {},
      today: "2026-08-30",
    });
  ...
  test("no data at all yields no pricing and a warning (plugin maps it to $0)", () => {
    const { models, warnings } = resolveCatalog([model("totally-unknown")], {
      seed: {},
      overrides: {},
      today: "2026-08-30",
    });
...
describe("resolveCatalog hardening (code review fixes)", () => {
  ...
  test("an override key that matches no model warns instead of applying silently", () => {
    ...
    expect(
      warnings.some((w) => w.includes("matched no model in the live list")),
    ).toBe(true);
  });
  ...
  test("marketplace tiebreak is deterministic regardless of seed key order", () => {
    ...
    const run = (seed: any) =>
      resolveCatalog([model("gpt-oss:120b")], {
        seed,
        overrides: {},
        today: "2026-08-30",
      }).models[0].pricing?.output;
  ...
  test("the real overrides file is well-formed (every entry consumable)", async () => {
    const file = Bun.file(
      new URL("../catalog/pricing-overrides.json", import.meta.url),
    );
