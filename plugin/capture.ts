...
import { createStatsCollector, measurementFromWire, type StatsCollector } from "./stats.ts"
import { createHandoffStore, MAX_HANDOFF_STEPS, type HandoffStore } from "./handoff.ts"
...
export interface StatsCapture {
  ...
  wireFetch: (input: RequestInfo | URL, init?: RequestInit, next?: FetchLike) => Promise<Response>
  /** Server event hook body (never throws). */
  handleEvent(event: unknown): Promise<void>
  store: HandoffStore
}

const toAssistantMessageInfo = (message: Record<string, unknown>, sessionID: string) => ({
  role: "assistant" as const,
  parentID: typeof message.parentID === "string" && message.parentID.length > 0 ? message.parentID : null,
...
const headerOf = (input: Parameters<typeof fetch>[0], init: RequestInit | undefined, name: string): string | null => {
...
export const createStatsCapture = (input: { now?: () => number; handoffDir?: string } = {}) => {
  const now = input.now ?? (() => Date.now())
  const store = createHandoffStore(input.handoffDir)
  const collectors = new Map<string, StatsCollector>()
  const sessions = new Map<string, { parentId: string | null; agent: string | null }>()
  ...
  const rememberSession = (id: string, entry: { parentId: string | null; agent: string | null }) => {
  ...
  const persist = async (sessionID: string): Promise<void> => {
    ...
    if (current && current.sessionID === sessionID && current.summary.steps > summary.steps) {
  ...
  const handleEvent = async (event: unknown): Promise<void> => {
    try {
      ...
      if (type === "session.created" || type === "session.updated") {
        ...
        const id = typeof (info as Record<string, unknown>).id === "string" ? ((info as Record<string, unknown>).id as string) : null
        if (!id) return
        const previous = sessions.get(id)
        rememberSession(id, { parentId: readStringField(info, "parentID"), agent: previous?.agent ?? null })
        return
      }
      ...
      if (type === "message.part.updated") {
        ...
        const messageID =
          part && typeof part === "object"
            ? readStringField(part, "messageID") ?? readStringField(part, "id")
            : null
        if (!messageID || firstPart.has(messageID)) return
        const partTime = (part as { time?: { start?: unknown } } | undefined)?.time?.start
        const at = [record.time, partTime].find((v): v is number => typeof v === "number" && v > 0)
      ...
      if (sessionInfo && sessionInfo.agent == null && messageInfo.agent) {
        rememberSession(sessionID, { ...sessionInfo, agent: messageInfo.agent })
      }
      ...
      console.warn("[opencode-ollama-cloud] stats capture ignored an event error:", error instanceof Error ? error.message : error)
    }
  }
  ...
  const ingestEventStep = async (sessionID: string, message: Record<string, unknown>): Promise<void> => {
    const messageID = readStringField(message, "id")
    if (!messageID) return
    const time = message.time as { created?: unknown; completed?: unknown } | undefined
    ...
    const tokensOut = typeof tokens?.output === "number" && tokens.output > 0 ? tokens.output : null
    if (created == null || completed == null || firstPartTime == null || tokensOut == null) return
    firstPart.delete(messageID)
    ...
    collectorFor(sessionID).ingest(measurement, {
      ...
      session: {
        ...
        agent: sessions.get(sessionID)?.agent ?? readStringField(message, "agent"),
      },
  ...
  const wireFetch = async (input: RequestInfo | URL, init?: RequestInit, next?: FetchLike): Promise<Response> => {
    const upstream = next ?? ((input2: RequestInfo | URL, init2?: RequestInit) => fetch(input2, init2))
    ...
    if (headerOf(input, init, PARENT_SESSION_HEADER)) return upstream(input, init)
    const sessionID = headerOf(input, init, SESSION_HEADER)
    if (!sessionID) return upstream(input, init)
    return measureSseFetch(sessionID, upstream, input as RequestInfo | URL, init)
  }
  ...
  const measureSseFetch = async (sessionID: string, next: FetchLike, input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    const t0 = now()
    ...
    const finish = () => {
      if (measured) return
      measured = true
      const measurement = measurementFromWire({ t0, t2: now(), chunks, providerID: "ollama-cloud", modelID: "unknown" })
      if (!measurement) return
      collectorFor(sessionID).pend(
        { ttftMs: measurement.ttftMs, tokensOut: measurement.tokensOut, decodeMs: measurement.decodeMs, source: measurement.source, ts: t0 },
        { sessionID, requestParentSessionId: null, sessionParentId: sessions.get(sessionID)?.parentId ?? null, sessionModelID: null, now: t0 },
      )
    }
    ...
    const consume = (raw: string) => {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          const usage = (parsed as { usage?: { completion_tokens?: unknown } } | null)?.usage
          if (usage && typeof usage.completion_tokens === "number" && chunks.length > 0)
            chunks[chunks.length - 1].usage = { completion_tokens: usage.completion_tokens }
        } catch {
