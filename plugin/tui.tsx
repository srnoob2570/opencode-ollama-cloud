...
const ZERO_SUMMARY: SessionSummary = { steps: 0, tokensOutTotal: 0, decodeMsTotal: 0, avgTps: 0, avgTtftMs: 0 }
...
const debug = (...message: unknown[]) => {
  try {
    appendFileSync(DEBUG_FILE, `[${new Date().toISOString()}] ${message.map(String).join(" ")}\n`)
...
const showStats = async (api: TuiLike): Promise<void> => {
  try {
    ...
    const body = formatStatsDialogBody(file?.summary ?? ZERO_SUMMARY, file?.steps ?? [], file?.steps[0]?.modelID ?? "—", Date.now())
    api.ui.dialog.replace(() =>
      api.ui.DialogAlert({ title: "/stats", message: body, onConfirm: () => api.ui.dialog.clear() }),
    )
  } catch (error) {
    console.warn("[opencode-ollama-cloud/tui] /stats failed silently:", error instanceof Error ? error.message : error)
    api.ui.dialog.clear()
  }
}
  ...
  try {
    ...
    if (modelID) {
      try {
        ...
        const model: CatalogModel2["models"][number] | undefined = catalog?.models.find((m) => m.id === modelID)
        if (model) {
    ...
    console.warn("[opencode-ollama-cloud/tui] /model failed silently:", error instanceof Error ? error.message : error)
    api.ui.dialog.clear()
  }
}
...
export default {
  ...
  async tui(api: TuiLike, options?: { stats?: string; pricing?: string }) {
    debug("tui module entry, version", String((api as { app?: { version?: string } }).app?.version ?? "?"))
    try {
      ...
      const features: { slots: boolean; keymap: boolean } = pickTuiFeatures(api)
      ...
      const refresh = async (sessionID?: string) => {
        try {
          ...
          if (next !== lastLine) {
            debug(
              "line:", "active=", String(sessionID ?? activeSessionID ?? "none"),
              "steps=", String(file?.summary?.steps ?? -1), "->", next,
            )
            lastLine = next
          }
          setLine(next)
        } catch (error) {
          debug("refresh error:", error instanceof Error ? error.message : String(error))
        }
      }
        ...
        slots: {
          session_prompt_right: (_ctx: unknown, props: { session_id?: string }) => {
            try {
              ...
              debug("slot render error:", error instanceof Error ? error.stack ?? error.message : String(error))
              return null
            }
          },
        },
      ...
      if (features.keymap) {
        ...
        const pricingOn = referencePricingActive(api.state?.config, { pricing: options?.pricing })
      ...
      debug("tui() threw:", error instanceof Error ? error.stack ?? error.message : String(error))
      console.warn(
