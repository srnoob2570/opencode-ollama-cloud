...
describe("isCatalog optional quantization", () => {
  const validModel = catalogModel("glm-5.3", { context: 1024 * 1024, maxOutput: 131072 })
  ...
  test("raw values and the unknown literal load", () => {
    for (const value of ["FP8", "MXFP4", "unknown", "Some-New-Format-2049"])
      expect(isCatalog(catalogWith([{ ...validModel, quantization: value }]))).toBe(true)
  })

  test("garbage is rejected (empty string, numbers)", () => {
    expect(isCatalog(catalogWith([{ ...validModel, quantization: "" }]))).toBe(false)
    expect(isCatalog(catalogWith([{ ...validModel, quantization: 8 as unknown as string }]))).toBe(false)
  })
})
...
describe("isCatalog pricing compat (code review fixes)", () => {
  ...
  test("a foreign catalog with models.dev-shaped pricing ({input, output}) still loads", () => {
    expect(
      isCatalog(catalogWith([{ ...validModel, pricing: { input: 0.2, output: 0.7 } }])),
    ).toBe(true)
  })
