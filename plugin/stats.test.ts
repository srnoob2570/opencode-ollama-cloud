...
const MAIN: Parameters<typeof isMainStep>[0] = {
  ...
  message: { role: "assistant", parentID: "um1", agent: "build", mode: null, summary: null, modelID: "glm-5.3" },
...
describe("summarize (promedio de sesión)", () => {
  test("weighted TPS: total tokens over total decode time, TTFT simple mean", () => {
    const s = summarize([step({ tokensOut: 600, decodeMs: 10_000, ttftMs: 400 }), step({ tokensOut: 300, decodeMs: 10_000, ttftMs: 200 })])
  ...
  test("short steps do not distort the weighted average (tool-loop steps)", () => {
    const s = summarize([step({ tokensOut: 1000, decodeMs: 20_000 }), step({ tokensOut: 2, decodeMs: 1_000 })])
    expect(s.avgTps).toBeCloseTo(1002 / 21, 2) // 1002 tokens / 21 s, not ((50 + 2) / 2)
  })

  test("empty session has zero shape without NaN", () => {
    expect(summarize([])).toEqual({ steps: 0, tokensOutTotal: 0, decodeMsTotal: 0, avgTps: 0, avgTtftMs: 0 })
  })
})
...
describe("isMainStep (regla del chat principal, spec §1.3)", () => {
  ...
  test("subagent wire request: child sessions always send x-parent-session-id", () => {
    expect(isMainStep({ ...MAIN, requestParentSessionId: "root-session" })).toBe(false)
  })

  test("subagent event route: session has parentID", () => {
    expect(isMainStep({ ...MAIN, session: { ...MAIN.session, parentID: "root" } })).toBe(false)
  })
  ...
  test("compaction is excluded by agent/mode/summary even when it inherits the session model", () => {
    for (const message of [
      ...
      { ...MAIN.message, mode: "compaction", agent: "compaction", summary: true },
    ])
      expect(isMainStep({ ...MAIN, message, sessionModelID: "glm-5.3" })).toBe(false)
  })

  test("agent of the message must be the session's active agent (no constants)", () => {
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, agent: "plan" } })).toBe(false)
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, agent: null } })).toBe(false)
    expect(isMainStep({ ...MAIN, session: { parentID: null, agent: null } })).toBe(false)
  })
  ...
  test("task-part wrapper message agents differently (agent ≠ session active agent)", () => {
    ...
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, agent: "task" } })).toBe(false)
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, parentID: "other-msg" } })).toBe(true)
  })
  ...
  test("modelID is a sanity check only (insufficient alone by design)", () => {
    ...
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, modelID: "kimi-k3" } })).toBe(false)
  })

  test("non-assistant roles never count", () => {
    expect(isMainStep({ ...MAIN, message: { ...MAIN.message, role: "user" } })).toBe(false)
  })
...
describe("claimPendingWire (titlegen se descarta por no correlacionar)", () => {
  const pend = {
    sessionID: "s1",
    measurement: { ttftMs: 300, tokensOut: 120, decodeMs: 4_000, source: "wire" as const, ts: 1000 },
    deadline: 31_000,
  ...
  test("attaches to the assistant message and passes the main-step gate", () => {
    const { claimed, rest } = claimPendingWire(
      [{ ...pend, sessionID: "s1" }],
      {
        ...
        sessionID: "s1",
        session: { parentID: null, agent: "build" },
        message: { role: "assistant", agent: "build", modelID: "glm-5.3", providerID: "ollama-cloud" },
      },
    )
  ...
  test("expired pendings (no message ever claims them = titlegen) are dropped by the window", () => {
    const { claimed } = claimPendingWire([{ ...pend, sessionID: "s1", deadline: 31_000 }], {
      now: 40_000,
      sessionID: "s1",
      session: { parentID: null, agent: "build" },
      message: { role: "assistant", agent: "build", modelID: "glm-5.3", providerID: "ollama-cloud" },
  ...
  test("other sessions' pendings stay untouched", () => {
    ...
    const { claimed, rest } = claimPendingWire([{ ...pend, sessionID: "s1" }, keep], {
      now: 2000,
      sessionID: "s1",
      session: { parentID: null, agent: "build" },
      message: { role: "assistant", agent: "build", modelID: "glm-5.3", providerID: "ollama-cloud" },
...
describe("StatsCollector (in-memory, vivo, sin persistencia)", () => {
  ...
  test("wire route: pend → claim promotes into steps", () => {
    const c = makeCollector()
    c.pend({ ttftMs: 250, tokensOut: 90, decodeMs: 3_000, source: "wire", ts: 5 }, {
      sessionID: "s1",
      requestParentSessionId: null,
      sessionParentId: null,
      sessionModelID: "glm-5.3",
  ...
  test("wire route: child sessions never pend (subagentes fuera, antes de correlacionar)", () => {
    const c = makeCollector()
    c.pend({ ttftMs: 250, tokensOut: 90, decodeMs: 3_000, source: "wire", ts: 6 }, {
      sessionID: "s1",
    ...
    c.claim({ now: 10, sessionID: "s1", session: { parentID: null, agent: "build" }, message: { role: "assistant", agent: "build" } })
    expect(c.summary().steps).toBe(0)
  })

  test("sweep drops stale pendings (titlegen never claims them)", () => {
    const c = makeCollector()
    c.pend({ ttftMs: 250, tokensOut: 12, decodeMs: 500, source: "wire", ts: 6 }, {
      sessionID: "s1", requestParentSessionId: null, sessionParentId: null, sessionModelID: "glm-5.3", now: 6,
    })
    c.sweep(999_999)
    c.claim({ now: 999_001, sessionID: "s1", session: { parentID: null, agent: "build" }, message: { role: "assistant", agent: "build" } })
    expect(c.summary().steps).toBe(0)
  })
...
describe("measurementFromWire (extracción pura del stream)", () => {
  ...
  test("no usage anywhere → no measurement (tokens unknown, never guessed)", () => {
    expect(
      measurementFromWire({
        t0: 0, t2: 100, chunks: [{ t: 50 }], providerID: "p", modelID: "m",
      }),
    ).toBeNull()
  })
})
describe("claimPendingWire — contrato newest-wins", () => {
  const pend = (ts: number, over: Record<string, unknown> = {}) => ({
    sessionID: "s1",
    measurement: { ttftMs: 300 + ts, tokensOut: 100 + ts, decodeMs: 4_000, source: "wire" as const, ts },
